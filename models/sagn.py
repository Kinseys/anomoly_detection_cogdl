...
class SAGN(BaseModel):
    ...
    def __init__(
        ...
        self.label_mlp = nn.ModuleList(
            [
                MLP(2, out_feats, hidden_size, mlp_layer, norm="batchnorm", dropout=dropout)
                for _ in range(nhop + 1)
            ]
        )
    ...
    def reset_parameters(self):
        ...
        if self.use_labels:
            self.label_mlp.nn.reset_parameters()
        self.norm.reset_parameters()
        self.mlp.nn.reset_parameters()

    def forward(self, graph, y_emb=True):
        features = graph.x
        y_emb = graph.y_emb

        out = 0
        features = [F.dropout(x.clone().detach().requires_grad_(True), p=self.input_dropout, training=self.training) for x in features]
